...
from utils.data import DictArray
...
if __name__ == "__main__":
    ...
    if args.capture_video:
        ...
        if args.save_train_video_freq is not None:
            save_video_trigger = lambda x: (x // args.num_steps
                                            ) % args.save_train_video_freq == 0
            envs = RecordEpisode(
                envs,
                output_dir=f"runs/{args.run_name}/train_videos",
    ...
    obs = DictArray((args.num_steps, args.num_envs),
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        final_values = torch.zeros((args.num_steps, args.num_envs),
                                   device=device)
        agent.eval()
        ...
        if args.save_model and iteration % args.eval_freq == 1:
            model_path = f"runs/{args.run_name}/ckpt_{iteration}.pt"
        ...
        for step in range(0, args.num_steps):
            ...
            next_obs, reward, terminations, truncations, infos = envs.step(
                action)
            next_done = torch.logical_or(terminations,
                                         truncations).to(torch.float32)
            ...
            if "final_info" in infos:
                ...
                for k, v in final_info["episode"].items():
                    logger.add_scalar(f"train/{k}",
                                      v[done_mask].float().mean(), global_step)

                for k in infos["final_observation"]:
                    infos["final_observation"][k] = infos["final_observation"][
                        k][done_mask]
                with torch.no_grad():
                    final_values[step,
                                 torch.arange(args.num_envs, device=device)
                                 [done_mask]] = agent.get_value(
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                real_next_values = next_not_done * nextvalues + final_values[
                    t]  # t instead of t+1
                ...
                if args.finite_horizon_gae:
                    ...
                    reward_term_sum = args.gae_lambda * args.gamma * reward_term_sum + lam_coef_sum * rewards[
                        t]
                    value_term_sum = args.gae_lambda * args.gamma * value_term_sum + args.gamma * real_next_values

                    advantages[t] = (reward_term_sum +
                                     value_term_sum) / lam_coef_sum - values[t]
                else:
                    delta = rewards[
                        t] + args.gamma * real_next_values - values[t]
                    advantages[
        ...
        logger.add_scalar("losses/old_approx_kl", old_approx_kl.item(),
                          global_step)
        logger.add_scalar("losses/approx_kl", approx_kl.item(), global_step)
        logger.add_scalar("losses/clipfrac", np.mean(clipfracs), global_step)
        logger.add_scalar("losses/explained_variance", explained_var,
                          global_step)
        print("SPS:", int(global_step / (time.time() - start_time)))
        logger.add_scalar("charts/SPS",
        ...
        logger.add_scalar(
            ...
            global_step)
    if args.save_model:
        model_path = f"runs/{args.run_name}/final_ckpt.pt"
        torch.save(agent.state_dict(), model_path)
        print(f"model saved to {model_path}")
