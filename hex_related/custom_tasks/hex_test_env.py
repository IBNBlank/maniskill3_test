...
class PushCubeEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    ##############################
    # Specify default sensor configurations
    # ----------------------------
    ## Optional
    ##############################
    @property
    ...
    ##############################
    # Specify default human render camera configurations
    # ----------------------------
    ## Optional
    ##############################
    @property
    ...
    ##############################
    # Loading
    # Generally only ever occurs once
    # ----------------------------
    ## build robots
    ## required
    ##############################
    ...
    # Called whenever `env.step()` is called
    # Used to evaluate that the task is in a fail or success state
    # ----------------------------
    ## required
    ##############################
    def evaluate(self):
        ...
        is_obj_placed = (torch.linalg.norm(
            self.obj.pose.p[..., :2] - self.goal_region.pose.p[..., :2],
            axis=1) < self.goal_radius) & (self.obj.pose.p[..., 2]
                                           < self.cube_half_size + 5e-3)
    ...
    ##############################
    # Extra Observation
    # Only called when `obs_mode` is "state_dict" or "state"
    # Called whenever `env.step()` is called
    # ----------------------------
    ## required
    ##############################
    ...
    # Only called when `reward_mode` is "dense"
    # Called whenever `env.step()` is called
    # ----------------------------
    ## Optional
    ##############################
    def compute_dense_reward(self, obs: Any, action: Array, info: dict):
        ...
        tcp_push_pose = Pose.create_from_pq(p=self.obj.pose.p + torch.tensor(
            [-self.cube_half_size - 0.005, 0, 0], device=self.device))
        ...
        obj_to_goal_dist = torch.linalg.norm(self.obj.pose.p[..., :2] -
    ...
    ## Optional
    ##############################
    def compute_normalized_dense_reward(self, obs: Any, action: Array,
                                        info: dict):
        # this should be equal to compute_dense_reward / max possible reward
        max_reward = 4.0
        return self.compute_dense_reward(obs=obs, action=action,
                                         info=info) / max_reward
