...
@register_env("AssistPickSingleYCB-v1",
              max_episode_steps=100,
              asset_download_ids=["ycb"])
class AssistPickSingleYCB(BaseEnv):
    """
    **Task Description:**
    ...
    **Additional Notes**
        ...
        **kwargs,
        ...
        self.all_model_ids = np.array([
            k for k in load_json(
                ASSET_DIR / "assets/mani_skill2_ycb/info_pick_v0.json").keys()
            if k not in [
                "022_windex_bottle",
                "028_skillet_lid",
                "029_plate",
        ...
        with torch.device(self.device):
            ...
            if self.robot_uids == "panda" or self.robot_uids == "panda_wristcam":
                ...
                qpos[:-2] += self._episode_rng.normal(
                    0, self.robot_init_qpos_noise,
                    len(qpos) - 2)
                self.agent.reset(qpos)
                self.agent.robot.set_root_pose(sapien.Pose([-0.615, 0, 0]))

                tar_pose = Pose(self.agent.tcp.pose.raw_pose[env_idx])
                self.goal_site.set_pose(tar_pose)
            else:
                raise NotImplementedError(self.robot_uids)

    def evaluate(self):
        obj_to_goal_pos = self.goal_site.pose.p - self.obj.pose.p
        is_obj_placed = torch.linalg.norm(obj_to_goal_pos,
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        tcp_to_obj_dist = torch.linalg.norm(self.obj.pose.p -
        ...
        static_reward = 1 - torch.tanh(5 * torch.linalg.norm(
            self.agent.robot.get_qvel()[..., :-2], axis=1))
